...
contract LaLoHotelRegistryTest is Test {
    ...
    function testRegisterHotelAndVault() public {
        ...
        assertEq(vault.usdcToken().balanceOf(vaultAddress), 0, "USDC balance should be 0");
        ...
        assertEq(vault.totalRevenue(), tokenAmount, "Vault total revenue should match");
        ...
        assertEq(vault.promisedRevenue(), tokenAmount, "Vault promised revenue should match");

        assertEq(vault.remainingPromisedRevenue(), tokenAmount, "Vault remaining promised revenue should match");
        ...
        assertEq(vault.getAvailableTokens(), 900, "Vault should lose 100 tokens");

        // Check Alice's USDC balance
        assertEq(usdc.balanceOf(alice), 0, "Alice should have 0 USDC");

        // Check Alice's LLoT balance
        assertEq(token.balanceOf(alice), 100, "Alice should have 100 LLoT tokens");

        // Check Alice's LLoT balance from vault
        assertEq(vault.checkBalance(alice), 100, "Vault should have 100 LLoT tokens for Alice");

        // Check Alice's withdraw limit
        assertEq(vault.getTransferLimit(alice), 0, "Alice's withdraw limit should be 0");

        // Check Alice's claimed revenues
        assertEq(vault.getClaimedRevenues(alice), 0, "Alice's claimed revenues should be 0");
        ...
        assertEq(vault.getTransferLimit(alice), 10, "Transfer limit must change to 10 due to monthly shares");
        ...
        assertEq(vault.getTransferLimit(alice), 6, "Alice's transfer limit must be 6 left");

        // Vault's balance must lose 4 usdc
        assertEq(vault.getAvailableRevenues(), 86, "Available revenues should lose 4 USDC");
        ...
        // Check limit
        assertEq(vault.getTransferLimit(alice), 0, "Alice's transfer limit must be 0 due to full withdraw");

        // Check claimed revenues
        assertEq(vault.claimedRevenuesInLLoT(alice), 50, "Alice should've withdrawn 46 + 4 USDC");

        // Check revenue
        assertEq(vault.getAvailableRevenues(), 40, "Available revenues should lose 46 + 4 USDC");

        // Warp to more than the time limit
        vm.warp(block.timestamp + 2_592_000 * 10);

        // Check Alice's transfer limit
        assertEq(vault.getTransferLimit(alice), 50, "Alice's transfer limit must be 50 due to remaining");
        ...
        assertEq(vault.claimedRevenuesInLLoT(alice), 100, "Alice should've withdrawn 46 + 4 + 50 USDC");
